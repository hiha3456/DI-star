...
srun --partition=GAME --quotatype spot -N 2 --gres=gpu:8 --ntasks-per-node 8 -c 32 --job-name=learner_learner_speed --mpi=pmi2 --async --output=./check-learner-speed-logs/learner_output_check_learner_speed.txt python -m distar.bin.rl_train --type learner --coordinator_ip $2